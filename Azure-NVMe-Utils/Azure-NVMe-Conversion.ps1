...
try {
    $extension = Get-AzVMExtension -ResourceGroupName $VMName -VMName $ResourceGroupName -Name "AzureDiskEncryptionForLinux" -ErrorAction Stop

    if ($extension.ProvisioningState -eq "Succeeded") {
        WriteRunLog -message "ADE extension is installed and succeeded on VM: $($extension.VMName)" -category "ERROR"
		WriteRunLog -message "Azure Disk Encryption don't support NVMe disks" -category "ERROR"
		WriteRunLog $_.Exception.Message "ERROR"
        exit
    } else {
        WriteRunLog -message "ADE extension is installed but provisioning state is: $($extension.ProvisioningState)" -category "ERROR"
